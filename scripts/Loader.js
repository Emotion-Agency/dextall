...
export class Loader {
  constructor(cb) {
    ...
    this.sPli = document.querySelector('[data-spli]')
    this.cb = cb ?? noop
  ...
  loadSequence() {
    const loaded = +this.sPli.getAttribute('data-loaded')
    this.counter(+loaded.toFixed(0), 1)
  }

  afterLoad() {
    if (!this.state) {
      this.state = true
      raf.off(this.loadSequence)
    ...
    return new Promise(resolve => {
      gsap.to(this.num, {
        ...
        onUpdate: () => {
          ...
          this.percentLine.style.height = result
        },
        onComplete: () => {
          if (count !== 100) {
            resolve()
